    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 space-y-6">
        ...
        <UCard>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <UFormField label="Select Student" required>
              <USelect v-model="formState.studentId" :items="students.map(s => ({ label: `${s.firstname} ${s.lastname}`, value: s.id }))" placeholder="Choose a student" />
            </UFormField>
            <UFormField label="Assessment Date" required>
              <UInput v-model="formState.assessmentDate" type="date" />
            </UFormField>
          </div>
          <div class="mt-4">
            <UFormField label="Select Question Groups">
              <div class="flex gap-2">
                <USelect 
                  ...
                  :items="allQuestionGroups.map(g => ({ label: `${g.title} (${g.domain})`, value: g.id }))"
        ...
        <div class="flex justify-end gap-3">
          <UButton color="gray" variant="ghost" @click="router.push('/assessments')">Cancel</UButton>
          <UButton color="primary" :loading="submitting" :disabled="Object.keys(domainScores).length === 0" @click="saveAssessment">Save Assessment</UButton>
        </div>
      </div>
